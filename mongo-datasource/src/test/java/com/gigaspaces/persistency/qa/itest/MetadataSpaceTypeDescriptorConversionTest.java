...
public class MetadataSpaceTypeDescriptorConversionTest extends
		AbstractMongoTest {
	...
	private SpaceTypeDescriptor createTypeDescriptor(
		...
		return builder
				...
				.addFixedProperty("typedProperty",
						MyCassandraPojoWithPrimitives.class)
				.addFixedProperty("propertyDocumentSupportConvert",
						MyCassandraPojo1.class, SpaceDocumentSupport.CONVERT)
				.addFixedProperty("propertyDocumentSupportCopy",
						MyCassandraPojo2.class, SpaceDocumentSupport.COPY)
				.addFixedProperty("propertyDocumentSupportDefault",
						MyCassandraPojo2.class, SpaceDocumentSupport.DEFAULT)
				.addFixedProperty("propertyStorageTypeDefault",
						MyCassandraPojo3.class, StorageType.DEFAULT)
				.addFixedProperty("propertyStorageTypeObject",
						MyCassandraPojo3.class, StorageType.OBJECT)
				.addFixedProperty("propertyStorageTypeBinary",
						MyCassandraPojo4.class, StorageType.BINARY)
				.addFixedProperty("propertyStorageTypeCompressed",
						MyCassandraPojo4.class, StorageType.COMPRESSED)
				...
				.idProperty("idPropertyName", true).replicable(true)
				.routingProperty("routing.property.name")
				.storageType(StorageType.OBJECT)
				.supportsDynamicProperties(true)
				.supportsOptimisticLocking(true).create();
	}

	private void testTypeDescriptor(SpaceTypeDescriptor typeDescriptor) {
		_syncInterceptor.onIntroduceType(new MockIntroduceTypeData(
				typeDescriptor));
		DataIterator<SpaceTypeDescriptor> dataIterator = _dataSource
				.initialMetadataLoad();
		while (dataIterator.hasNext()) {
			SpaceTypeDescriptor readTypeDescriptor = dataIterator.next();
			if (!readTypeDescriptor.getTypeName().equals(
					typeDescriptor.getTypeName()))
				continue;

			TestSpaceTypeDescriptorUtils.assertTypeDescriptorsEquals(
					typeDescriptor, readTypeDescriptor);
			return;
		}
		Assert.fail("Could not find metadata for "
				+ typeDescriptor.getTypeName());
	}
