...
public class MultiTypeMongoTest extends AbstractMongoTest {
	...
	public void before() {
		_syncInterceptor
				.onIntroduceType(createIntroduceTypeDataFromSpaceDocument(
						createSpaceDocument1(false), document1KeyName));
		_syncInterceptor
				.onIntroduceType(createIntroduceTypeDataFromSpaceDocument(
						createSpaceDocument2(false), document2KeyName));
		_syncInterceptor
				.onIntroduceType(createIntroduceTypeDataFromSpaceDocument(
						createSpaceDocument3(false), document3KeyName));
	...
	public void test() {
		...
		Assert.assertEquals("count differs", batchCount * thirdBatchSize * 3,
				count);
	...
	private SpaceDocument createSpaceDocument1(boolean addToWrittenKeys) {
		...
		return new SpaceDocument("TypeName").setProperty(document1KeyName, key)
	...
	private SpaceDocument createSpaceDocument2(boolean addToWrittenKeys) {
		...
		return new SpaceDocument("TypeName2")
				...
				.setProperty("payload", bytes).setProperty("payload2", bytes);
	}
