...
public class AdvanceQueriesMongoTest extends AbstractMongoTest {
	...
	private static final String DYNAMIC_COL_9_NAME = DYNAMIC_COL_9 + ".name";
	...
	public void before() {
		...
		_syncInterceptor
				.onOperationsBatchSynchronization(createMockOperationBatchData());
	...
	public void test() {
		...
		testDataIterator(new SpaceDocument(TEST_CF).setProperty(LONG_COL,
				LONG_COL_VAL + 10), "<");
		testDataIterator(
				new SpaceDocument(TEST_CF).setProperty(STRING_COL, "test.*ing"),
				"rlike");

		testDataIterator(
				new SpaceDocument(TEST_CF).setProperty(STRING_COL, "%stStr%"),
				"like");

		testDataIterator(new SpaceDocument(TEST_CF).setProperty(BIGINT_COL,
				BIGINT_COL_VAL), "=");

		testDataIterator(new SpaceDocument(TEST_CF).setProperty(DYNAMIC_COL_1,
				DYNAMIC_COL_1_VAL - 10), ">");
		testDataIterator(new SpaceDocument(TEST_CF).setProperty(DYNAMIC_COL_2,
				DYNAMIC_COL_2_VAL), "<=");
		testDataIterator(new SpaceDocument(TEST_CF).setProperty(DYNAMIC_COL_3,
				DYNAMIC_COL_3_VAL), "=");
		testDataIterator(new SpaceDocument(TEST_CF).setProperty(DYNAMIC_COL_4,
				DYNAMIC_COL_4_VAL - 10), ">=");
		testDataIterator(new SpaceDocument(TEST_CF).setProperty(DYNAMIC_COL_5,
				DYNAMIC_COL_5_VAL), "=");
		testDataIterator(new SpaceDocument(TEST_CF).setProperty(DYNAMIC_COL_6,
				DYNAMIC_COL_6_VAL), "=");
		testDataIterator(new SpaceDocument(TEST_CF).setProperty(DYNAMIC_COL_7,
				DYNAMIC_COL_7_VAL), "=");
	...
	// construction
	private void testDataIterator(SpaceDocument document, String... op) {
		Map<String, Object> properties = document.getProperties();
		int i = 0;
		createMockSpaceTypeDescriptor();
		StringBuilder queryBuilder = new StringBuilder();
		for (String key : properties.keySet())
			queryBuilder.append(key).append(" " + op[i++] + " ? AND ");
		...
		DataSourceQuery dataSourceQuery = new MockDataSourceQuery(
				typeDescriptor, sqlQuery, Integer.MAX_VALUE);
	...
	private void assertValidDocument(SpaceDocument result) {
		...
		assertEquals("stringCol wrong", STRING_COL_VAL,
				result.getProperty(STRING_COL));
		assertEquals("longCol wrong", LONG_COL_VAL,
				result.getProperty(LONG_COL));
		assertEquals("bigintCol wrong", BIGINT_COL_VAL,
				result.getProperty(BIGINT_COL));
		assertEquals("dynamicCol1 wrong", DYNAMIC_COL_1_VAL,
				result.getProperty(DYNAMIC_COL_1));
		assertEquals("dynamicCol2 wrong", DYNAMIC_COL_2_VAL,
				result.getProperty(DYNAMIC_COL_2));
		assertEquals("dynamicCol3 wrong", DYNAMIC_COL_3_VAL,
				result.getProperty(DYNAMIC_COL_3));
		assertEquals("dynamicCol4 wrong", DYNAMIC_COL_4_VAL,
				result.getProperty(DYNAMIC_COL_4));
		assertEquals("dynamicCol5 wrong", DYNAMIC_COL_5_VAL,
				result.getProperty(DYNAMIC_COL_5));
		assertEquals("dynamicCol6 wrong", DYNAMIC_COL_6_VAL,
				result.getProperty(DYNAMIC_COL_6));
		assertEquals("dynamicCol7 wrong", DYNAMIC_COL_7_VAL,
				result.getProperty(DYNAMIC_COL_7));
		assertEquals("dynamicCol8 wrong", DYNAMIC_COL_8_VAL[0],
				((byte[]) result.getProperty(DYNAMIC_COL_8))[0]);
		assertEquals("dynamicCol9 wrong", DYNAMIC_COL_9_VAL,
				result.getProperty(DYNAMIC_COL_9));
	}
	...
	private OperationsBatchData createMockOperationBatchData() {
		...
		return new MockOperationsBatchDataBuilder().write(spaceDoc, KEY_NAME)
				.build();
	}
