...
public class DataIteratorWithPropertyAddedLaterMongoTest extends
		AbstractMongoTest {
	...
	@Before
	public void before() {
		introduceDataType = createIntroduceTypeDataFromSpaceDocument(
				createSpaceDocument(), keyName);
	...
	public void test() {
		...
		iterator = _dataSource.getDataIterator(new MockDataSourceQuery(
				introduceDataType.getTypeDescriptor(), new SpaceDocument(
						typeName), Integer.MAX_VALUE));
		...
		builder.write(createSpaceDocument().setProperty("new_prop", newType)
		...
		// uncomment if we decide pojo will be restored as documents no matter
		// what
		Assert.assertEquals("Wrong value", newType, doc.getProperty("new_prop"));
		// Assert.assertEquals("Wrong value", newType.getStr(),
		// ((SpaceDocument)doc.getProperty("new_prop")).getProperty("str"));
	...
	private SpaceDocument createSpaceDocument() {
		return new SpaceDocument(typeName).setProperty(keyName, 1).setProperty(
				"some_prop", true);
	}
