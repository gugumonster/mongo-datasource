...
public class ReadByIdsMongoTest extends AbstractMongoTest {
	...
	public void test() {
		...
		DataIterator<Object> resultsIterator = _dataSource
				.getDataIteratorByIds(new DataSourceIdsQuery() {
					public SpaceTypeDescriptor getTypeDescriptor() {
						return introduceDataType.getTypeDescriptor();
					}

					public Object[] getIds() {
						return new Object[] { 0, 1, 2 }; // 0 doesn't exist, 1
															// and 2 exist
					}
		...
		Assert.assertEquals("bad document", somePropValue,
				doc1.getProperty(someProp));
		Assert.assertEquals("bad document", somePropValue,
				doc2.getProperty(someProp));

	}

	private SpaceDocument createSpaceDocument() {
		return new SpaceDocument(typeName).setProperty(keyName,
				keyValues.getAndIncrement()).setProperty(someProp,
				somePropValue);
	}
